...
namespace GamepadInput
{
    ...
    public static class GamePad
    {
        ...
        public static Vector2 GetAxis(Axis axis, Index controlIndex, bool raw = false)
        {
            ...
            try
            {
				//axisXY.x = GamePad.GetAxis(GamePad.Axis.LeftStick, controlIndex, raw).x;
				//axisXY.y = -GamePad.GetAxis(GamePad.Axis.LeftStick, controlIndex, raw).y;


				if (raw == false)
                {
					axisXY.x = Input.GetAxis(xName);//"Horizontal");
					axisXY.y = -Input.GetAxis(yName);//"Vertical");
                }
                else
                {
					axisXY.x = Input.GetAxisRaw(xName);//"Horizontal");
					axisXY.y = -Input.GetAxisRaw(yName);//"Vertical");
                } 
            }
